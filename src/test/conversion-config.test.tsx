...
describe('ConversionConfig', () => {
  ...
  it('should render basic options with default values', () => {
    ...
    const videoCodecSelect = screen.getByDisplayValue(
      'H.265 (HEVC) - Best compression',
    );
    expect(videoCodecSelect).toBeInTheDocument();

    // Check output format
    const outputFormatSelect = screen.getByDisplayValue(
      'MP4 - Most compatible',
    );
  ...
  it('should show/hide advanced options when toggled', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('Encoding Preset (Speed vs Compression)'),
      ).toBeInTheDocument();
    });
  });

  it('should render quality slider in advanced options with default value', async () => {
    render(<ConversionConfig {...defaultProps} />);
    ...
    // Wait for advanced options to be visible and check quality slider
    await waitFor(() => {
  ...
  it('should update basic options and call onOptionsChange', async () => {
    ...
    const lastCall =
      mockOnOptionsChange.mock.calls[
        mockOnOptionsChange.mock.calls.length - 1
      ][0];
  ...
  it('should update quality slider and call onOptionsChange', async () => {
    ...
    // Wait for advanced options to be visible, then interact with quality slider
    await waitFor(() => {
  ...
  it('should disable quality slider when codec is copy', async () => {
    ...
    // Change codec to copy
    const videoCodecSelect = screen.getByLabelText('Video Codec');
    fireEvent.change(videoCodecSelect, { target: { value: 'copy' } });

    // Expand advanced options to see the quality slider
    const advancedToggle = screen.getByText('Advanced Options');
    fireEvent.click(advancedToggle);
  ...
  it('should hide audio bitrate when codec is copy', async () => {
    ...
    await waitFor(() => {
      const audioCodecSelect = screen.getByDisplayValue(
    ...
    await waitFor(() => {
      expect(
        screen.queryByPlaceholderText('Audio bitrate (kbps)'),
      ).not.toBeInTheDocument();
    });
  });

  it('should handle custom FFmpeg command when expanded', async () => {
    render(<ConversionConfig {...defaultProps} />);

    // Custom FFmpeg options should be hidden initially
    expect(
      screen.queryByPlaceholderText('-tune film -profile:v high -level 4.1'),
    ).not.toBeInTheDocument();

    // Click to show custom FFmpeg options
    const customToggle = screen.getByText('Custom FFmpeg Options');
    fireEvent.click(customToggle);

    // Now the custom command textarea should be visible
    await waitFor(() => {
      const customCommandTextarea = screen.getByPlaceholderText(
        '-tune film -profile:v high -level 4.1',
      );
      expect(customCommandTextarea).toBeInTheDocument();
    });

    const customCommandTextarea = screen.getByPlaceholderText(
      '-tune film -profile:v high -level 4.1',
    );
    const customCommand = '-c:v libx264 -crf 20';

    fireEvent.change(customCommandTextarea, {
      target: { value: customCommand },
    });
  ...
  it('should load saved options from localStorage', async () => {
    const savedOptions = {
      basic: {
        videoCodec: 'libx264',
        quality: 20,
    ...
    // Check that saved basic options are applied
    expect(
      screen.getByDisplayValue('H.264 (AVC) - More compatible'),
    ).toBeInTheDocument();
    expect(screen.getByDisplayValue('MKV - Open standard')).toBeInTheDocument();

    // Expand advanced options to check quality value
    const advancedToggle = screen.getByText('Advanced Options');
    fireEvent.click(advancedToggle);
  ...
  it('should show quality range recommendations based on codec', async () => {
    ...
    // Expand advanced options to see quality recommendations
    const advancedToggle = screen.getByText('Advanced Options');
    fireEvent.click(advancedToggle);
    ...
    await waitFor(() => {
      expect(
        screen.getByText('20 for archival (lower = higher quality)'),
      ).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText('25 for archival (higher = higher quality)'),
      ).toBeInTheDocument();
    });
